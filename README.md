...
(*) questi file vengono scaricati dal repository con git

Il file `.env` contiene la configurazione delle porte su cui vengono esposti i servizi. Sono impostate in modo da non entrare in conflitto con le porte standard 80, 443 [TBD] e 3306. Modificale con i valori standard se queste porte non sono già occupate.
...
$ ./start.sh
```
Se preferisci avviare il tutto a mano, una tantum occorre creare il file per il log di mysql (che su Ubuntu per ora non funziona):
...
Visitare quindi [localhost:8080](localhost:8080) (correggi `8080` con la porta configurata nel file `.env`, se l'hai cambiata) per verificare il corretto funzionamento del tutto, la pagina dovrebbe rispondere con le scritte:
